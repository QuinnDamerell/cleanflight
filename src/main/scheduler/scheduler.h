...
typedef enum {
    TASK_PRIORITY_IDLE = 0,     // This task will only run if there is nothing else to run.
    TASK_PRIORITY_LOW = 1,   
    TASK_PRIORITY_MEDIUM = 3,
    TASK_PRIORITY_HIGH = 5,
    TASK_PRIORITY_REALTIME = 6, // This task will be ran as close to realtime as possible. 
...
typedef struct {
    ...
    uint32_t     desiredPeriod;             // the desired ammount of time between each execution (in microseconds)
    ...
    uint32_t     totalExecutionTime;        // keeps track of the total ammount of time this task has ran (in microseconds)
    uint32_t     averageExecutionTime;      // keeps track of the average run time (in microseconds)
    uint32_t     latestDeltaTime;           // keeps track of the last ammount of time between executions.
} cfTaskInfo_t;
...
typedef struct {
    ...
    // it will not be schedulled due to a period, instead when this funciton returns true the task will 
    // be schedulled. Note, once this function returns true it will not be called again until the task is ran.
    ...
    uint32_t desiredPeriod;         // If not an event based task, this is the target time (micros) between exections 
    ...
    uint32_t lastExecutedAt;        // last time (micros) when the task was ra

    /* Statistics */
    uint32_t averageExecutionTime;  // Moving average over 31 samples, used to calculate the realtime guard interval
    uint32_t taskLatestDeltaTime;   // keeps track of the last ammount of time between executions. 
...
// Preforms the scheduling logic exectures one task is possible.
...
// Updates the task period (micros) between exections. This does nothing if the task is event driven.
void updateTaskExecutionPeriod(const int taskId, uint32_t newPeriodMicros);

// Returns the last ammount of time between the execution of the given task.
